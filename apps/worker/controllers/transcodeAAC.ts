import { ReleaseContext } from "@gridfire/shared/types/index.js";
import { MessageType } from "@gridfire/shared/types/messages.js";
import { ffmpegEncodeFragmentedAAC, ffprobeGetTrackDuration } from "@gridfire/worker/controllers/ffmpeg.js";
import packageMP4 from "@gridfire/worker/controllers/packageMP4.js";
...
const transcodeAAC = async ({ releaseId, trackId, trackName, userId }: ReleaseContext) => {
  ...
  try {
    ...
    postMessage({ type: MessageType.TranscodingCompleteAAC, trackId, trackName, userId });
  } catch (error) {
    console.error(error);
