export const ADD_NEM_ADDRESS = 'ADD_NEM_ADDRESS';
export const ADD_RELEASE = 'ADD_RELEASE';
export const ADD_TRACK = 'ADD_TRACK';
export const DELETE_ARTWORK = 'DELETE_ARTWORK';
export const DELETE_RELEASE = 'DELETE_RELEASE';
export const DELETE_TRACK_START = 'DELETE_TRACK_START';
export const DELETE_TRACK_COMPLETE = 'DELETE_TRACK_COMPLETE';
export const FETCH_ARTIST_CATALOGUE = 'FETCH_ARTIST_CATALOGUE';
export const FETCH_CATALOGUE = 'FETCH_CATALOGUE';
export const FETCH_COLLECTION = 'FETCH_COLLECTION';
export const FETCH_INCOMING_TRANSACTIONS = 'FETCH_INCOMING_TRANSACTIONS';
export const FETCH_INCOMING_TRANSACTIONS_ERROR =
  'FETCH_INCOMING_TRANSACTIONS_ERROR';
export const FETCH_INCOMING_TRANSACTIONS_LOADING =
  'FETCH_INCOMING_TRANSACTIONS_LOADING';
export const FETCH_INCOMING_TRANSACTIONS_UPDATING =
  'FETCH_INCOMING_TRANSACTIONS_UPDATING';
export const FETCH_RELEASE = 'FETCH_RELEASE';
export const FETCH_SALES = 'FETCH_SALES';
export const FETCH_USER = 'FETCH_USER';
export const FETCH_USER_CREDIT = 'FETCH_USER_CREDIT';
export const FETCH_USER_RELEASE = 'FETCH_USER_RELEASE';
export const FETCH_USER_RELEASES = 'FETCH_USER_RELEASES';
export const FETCH_XEM_PRICE = 'FETCH_XEM_PRICE';
export const LOG_OUT = 'LOG_OUT';
export const MOVE_TRACK = 'MOVE_TRACK';
export const PLAY_TRACK = 'PLAY_TRACK';
export const PLAYER_HIDE = 'PLAYER_HIDE';
export const PLAYER_PAUSE = 'PLAYER_PAUSE';
export const PLAYER_PLAY = 'PLAYER_PLAY';
export const PLAYER_STOP = 'PLAYER_STOP';
export const PUBLISH_STATUS = 'PUBLISH_STATUS';
export const PURCHASE_RELEASE = 'PURCHASE_RELEASE';
export const SEARCH_RELEASES = 'SEARCH_RELEASES';
export const SEARCH_RELEASES_CLEAR = 'SEARCH_RELEASES_CLEAR';
export const SEARCH_RELEASES_LOADING = 'SEARCH_RELEASES_LOADING';
export const SET_IS_PAGING = 'SET_PAGING';
export const TOAST_ERROR = 'TOAST_ERROR';
export const TOAST_HIDE = 'TOAST_HIDE';
export const TOAST_INFO = 'TOAST_INFO';
export const TOAST_SUCCESS = 'TOAST_SUCCESS';
export const TOAST_WARNING = 'TOAST_WARNING';
export const TRANSCODING_DURATION = 'TRANSCODING_DURATION';
export const TRANSCODING_START = 'TRANSCODING_START';
export const TRANSCODING_STOP = 'TRANSCODING_STOP';
export const TRANSCODING_COMPLETE = 'TRANSCODING_COMPLETE';
export const UPDATE_RELEASE = 'UPDATE_RELEASE';
export const UPLOAD_ARTWORK = 'UPLOAD_ARTWORK';
export const UPLOAD_ARTWORK_COMPLETE = 'UPLOAD_ARTWORK_COMPLETE';
export const UPLOAD_ARTWORK_PROGRESS = 'UPLOAD_ARTWORK_PROGRESS';
export const UPLOAD_AUDIO_PROGRESS = 'UPLOAD_AUDIO_PROGRESS';
