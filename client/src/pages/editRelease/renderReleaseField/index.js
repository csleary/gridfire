...
const RenderReleaseField = ({ formText, input, label, meta: { error, touched }, min, type }) => {
  ...
  return (
    <div className={className}>
      ...
      {type === 'textarea' ? (
        <textarea className="form-control" id={input.name} rows="3" {...input} />
      ) : (
        <input className="form-control" id={input.name} type={type} min={min} {...input} />
      )}
