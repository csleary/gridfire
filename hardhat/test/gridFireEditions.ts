...
import { GridfireEditions } from "../typechain-types";
...
describe("GridfireEditions contract", async () => {
  let gridfireEditions: GridfireEditions;
  ...
  it("should NOT let buyer set editions address", async () => {
    ...
    const instanceEditions = gridfireEditions.connect(owner);
    const address = await instanceEditions.getAddress();
    const instance = gridfirePayment.connect(buyer);
    await expect(instance.setGridfireEditionsAddress(address)).to.eventually.be.rejected;
  });
  ...
  it("should let owner set editions address", async () => {
    ...
    const instanceEditions = gridfireEditions.connect(owner);
    const address = await instanceEditions.getAddress();
    const instance = gridfirePayment.connect(owner);
  ...
  it("should let artist mint edition", async () => {
    ...
    const instanceArtist = gridfireEditions.connect(artist);

    await instanceArtist.mintEdition(
      editionToMint.amount,
      editionToMint.price,
      editionToMint.uri,
      editionToMint.releaseId,
      editionToMint.objectId
    );
  ...
  it("should update buyer's dai allowance", async () => {
    ...
    const paymentInstance = gridfirePayment.connect(buyer);
  ...
  it("should let buyer purchase edition", async () => {
    const [, buyer, artist] = await ethers.getSigners();
    const instanceBuyer = gridfireEditions.connect(buyer);
