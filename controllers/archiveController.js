...
const zipDownload = async (res, release, format) => {
  ...
  const s3ListAudioMp3 = await s3.listObjectsV2({ Bucket, Prefix: releaseId }).promise();
  ...
  s3ListAudioMp3.Contents.forEach(s3Track => {
    ...
    const trackNumber = trackList.findIndex(track => Key.includes(track._id)) + 1;
  ...
  const s3ListArt = await s3.listObjectsV2({ Bucket: BUCKET_IMG, Prefix: releaseId }).promise();
